...
from pymelcloud import Ecodan, HeatPump
...
async def async_setup_entry(
    ...
    async_add_entities(
        [
            HeatPumpClimate(mel_device)
            for mel_device in mel_devices
            if isinstance(mel_device.device, HeatPump)
        ]
        + [
            EcodanZoneClimate(mel_device, zone_index)
            for mel_device in mel_devices
            if isinstance(mel_device.device, Ecodan)
            for zone_index in list(range(1, mel_device.device.zones + 1))
        ],
        True,
    )


class HeatPumpClimate(ClimateDevice):
    """MELCloud heat pump climate device."""

    def __init__(self, device: HeatPump, name=None):
        """Initialize the climate."""
        self._api = device
        if name is None:
            name = device.name

        self._name = f"{name} HVAC"
...
class EcodanZoneClimate(ClimateDevice):
    """MELCloud Ecodan zone climate device."""

    def __init__(self, device: Ecodan, zone_index: int, name=None):
        """Initialize the climate."""
        self._api = device
        if name is None:
            name = device.name
        ...
        self._zone = next(
            zone for zone in self._api.device.zones if zone.zone_index == zone_index
        )
        self._name = "{} {} {}".format(name, "Ecodan Zone", zone_index)

    @property
    def unique_id(self) -> Optional[str]:
        """Return a unique ID."""
        return f"{self._api.device.serial}-{self._api.device.mac}-zone{self._zone_index}-climate"
    ...
    def hvac_modes(self) -> List[str]:
        """Return the list of available hvac operation modes."""
        # return list(map(HVAC_MODE_LOOKUP.get, self._zone.operation_modes)
        return []
