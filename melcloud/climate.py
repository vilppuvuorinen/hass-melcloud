...
async def async_setup_entry(
	...
	async_add_entities(
		[MelCloudClimate(mel_device) for mel_device in mel_devices],
		True
	)
...
class MelCloudClimate(ClimateDevice):
	...
	def temperature_unit(self) -> str:
		...
		return TEMP_UNIT_LOOKUP.get(
			self._api.device.temp_unit,
			TEMP_CELSIUS
		)
	...
	def hvac_modes(self) -> List[str]:
		...
		return list(
			map(
				lambda x: HVAC_MODE_LOOKUP.get(x),
				self._api.device.operation_modes()
			)
		)
